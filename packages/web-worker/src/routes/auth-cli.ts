...
/**
 * `GET /api/auth/cli?callback_url=...&state=...`
 *
 * Loopback OAuth-style mint. The CLI starts a local listener on
 * `http://127.0.0.1:<port>` (or `localhost`), opens this URL, and we
 * 302 the browser back with `?api_key=pk_*&state=<echoed>`.
 *
 * Anti-abuse:
 *   - `callback_url` must be an absolute http URL with hostname
...
export async function authCliHandler(c: Context<AppEnv>) {
  ...
  const callbackRaw = c.req.query("callback_url");
  const state = c.req.query("state");

  if (!callbackRaw) return c.json({ error: "Missing callback_url" }, 400);
  ...
  if (callback.protocol !== "http:") {
    return c.json({ error: "callback_url must be http" }, 400);
  }
  if (callback.hostname !== "127.0.0.1" && callback.hostname !== "localhost") {
    return c.json({ error: "callback_url must be loopback" }, 400);
  }
