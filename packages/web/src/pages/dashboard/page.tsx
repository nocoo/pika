...
function HeatmapHero({ data, totalSessions, activeDays }: HeatmapHeroProps) {
...
export function DashboardPage() {
  ...
  const { data: sessionsData, error: sessionsError } =
    useSWR<SessionListResponse>("/api/sessions?limit=10", swrFetcher);

  const error = statsError ?? sessionsError;
  const loading = !stats || !sessionsData;
